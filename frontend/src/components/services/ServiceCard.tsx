...
export function ServiceCard({ service, summary, summaryError = false, index = 0 }: ServiceCardProps) {
  ...
  return (
    <article
      className="premium-surface service-card group rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.03)]"
      style={cardStyle}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="truncate font-semibold text-slate-950">{service.name}</h3>
            <a
              href={service.url}
              target="_blank"
              rel="noreferrer"
      ...
      <div className="mt-5 flex items-center gap-5 border-y border-slate-100 py-4">
        <div className="relative h-20 w-20 shrink-0 rounded-full p-[7px]" style={uptimeRing}>
          <div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white shadow-inner">
            <p className="metric-tabular text-sm font-bold tracking-tight text-slate-900">
              {hasData ? formatPercentage(summary?.uptime_percentage ?? 0) : "—"}
            </p>
        ...
        <div className="grid min-w-0 flex-1 grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="metric-tabular mt-1 text-sm font-semibold text-slate-800">
              {summaryError ? "—" : (summary?.total_checks.toLocaleString() ?? "—")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        ...
        <Link
          ...
          className="inline-flex shrink-0 items-center gap-1.5 text-xs font-semibold text-cyan-700 transition hover:text-cyan-600"
        >
          View details
